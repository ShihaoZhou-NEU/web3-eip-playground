...
const BurnerGame: React.FC<BurnerGameProps> = ({ onTutorSpeak }) => {
  ...
  const calculateNextBaseFee = (currentBaseFee: number, gasUsed: number) => {
    ...
    if (gasUsed === TARGET_GAS) {
      ...
      const baseFeeDelta = Math.max(
        1,
        Math.floor(
          (currentBaseFee * gasUsedDelta) /
            TARGET_GAS /
  ...
  const tick = () => {
    ...
    const actualUsagePercent = Math.min(
      1,
      Math.max(0, demandLevel / 100 + randomVariation)
    );
            ...
            <div className="bg-black/40 p-4 border-2 border-border/50">
              ...
              <div className="mb-4 space-y-2">
                <label className="block text-[10px] font-mono text-muted-foreground uppercase">
                  Max Fee Cap (Max Willing to Pay)
                </label>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="h-48 w-full bg-black/60 border-2 border-border p-2 relative">
              <div className="absolute top-2 right-2 z-10 flex gap-4 text-[10px] font-mono">
                ...
                <div className="flex items-center gap-1">
                  <div className="w-2 h-2 bg-red-500 border border-dashed border-white/50"></div>
                  Your Max Fee
                </div>
              </div>
              ...
              {isIncluded && (
                <div className="space-y-6">
                  ...
                  <div className="h-12 w-full flex border-2 border-border relative bg-card/30">
                    ...
                    >
                      ...
                      <span className="truncate px-1 relative z-10 flex items-center gap-1">
                        <Flame size={12} className="animate-pulse" /> BURN:{" "}
                        {burnt}
                      </span>
                    </div>
                    {/* Tip Part */}
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center font-mono text-sm">
                    ...
                    <div className="bg-card/50 p-3 border border-border/50">
                      ...
                      <div className="text-green-400 font-bold">
                        {(maxFee - actualPaid).toFixed(0)} Gwei
                      </div>
                    </div>
                    <div className="bg-card/50 p-3 border border-border/50">
                      <div className="text-muted-foreground text-[10px] uppercase mb-1">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-2 text-center border-t border-border/30 pt-2">
                    EIP-1559 Magic: You only pay Base Fee + Tip. Any extra Max
