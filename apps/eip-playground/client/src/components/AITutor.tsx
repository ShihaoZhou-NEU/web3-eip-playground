...
export default function AITutor({
  ...
  // Keep the latest callback without retriggering the typewriter effect.
    ...
    <>
      ...
      <div className="fixed bottom-16 right-2 sm:bottom-4 sm:right-4 md:right-6 z-50 w-40 sm:w-64 flex flex-col items-center pointer-events-none">
          ...
          <AnimatePresence>
            {displayedMessage && (
              <motion.div
                ...
                className="relative bg-white text-black p-3 sm:p-4 rounded-xl border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full pointer-events-auto"
              >
                <p className="font-mono text-[11px] sm:text-sm leading-relaxed">
                  {displayedMessage}
                  {isTyping && (
        ...
        <motion.div
          ...
          className="w-20 h-20 sm:w-40 sm:h-40 relative"
        >
      ...
      <AnimatePresence>
        {isChatOpen && (
          <motion.div
            ...
            className="fixed top-16 bottom-4 left-2 right-2 sm:top-20 sm:right-4 sm:left-auto sm:bottom-80 sm:w-96 bg-card border-4 border-black rounded-[10px] shadow-2xl z-100 flex flex-col"
          >
            {/* Header */}
            <div className="bg-primary text-white px-4 py-3 font-mono font-bold border-b-4 border-black rounded-t-[6px] flex items-center justify-between">
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
              {chatHistory.length === 0 ? (
                <p className="text-gray-500 text-center font-mono text-xs sm:text-sm mt-6 sm:mt-8">
                ...
                chatHistory.map(msg => (
                  ...
                  >
                    <div
                      className={`max-w-[90%] sm:max-w-[80%] p-3 rounded-lg font-mono text-xs sm:text-sm text-black ${
                        msg.role === "tutor"
