...
export const BatchingGame: React.FC = () => {
  const [mode, setMode] = useState<'intro' | 'eoa' | '7702' | 'success'>('intro');
  const [cards, setCards] = useState<number[]>(Array.from({ length: 10 }, (_, i) => i));
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-8 bg-gray-900 border-4 border-white font-pixel text-white relative overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,0.5)]">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-xl text-yellow-400 mb-2 text-shadow-pixel">LEVEL 1: CRAZY CLICKING</h2>
          <p className="text-xs text-gray-400 font-sans" style={{fontFamily: '"Press Start 2P", system-ui, sans-serif'}}>MISSION: REVEAL 10 CARDS</p>
        </div>
        <div className="text-right">
          <div className={`text-2xl ${timeLeft < 5 ? 'text-red-500 animate-pulse' : 'text-green-400'} text-shadow-pixel`}>
            {mode === 'eoa' ? `00:${timeLeft.toString().padStart(2, '0')}` : '--:--'}
          </div>
          <div className="text-xs text-gray-500 font-sans" style={{fontFamily: '"Press Start 2P", system-ui, sans-serif'}}>TIME LEFT</div>
        </div>
      </div>
      ...
      <div className="relative min-h-[500px] flex flex-col items-center justify-center bg-black/30 border-2 border-white/10 p-4">
        ...
        {mode === 'intro' && (
          <div className="text-center space-y-6">
            <p className="text-sm leading-relaxed max-w-lg mx-auto text-gray-300 font-sans" style={{fontFamily: '"Press Start 2P", system-ui, sans-serif'}}>
              In the old world (EOA), every action requires a separate transaction signature and confirmation wait time.
              <br/><br/>
        ...
        {(mode === 'eoa' || mode === '7702' || mode === 'success') && (
          <div className="grid grid-cols-5 gap-4 mb-8">
            {cards.map((card, index) => (
              <div
                key={index}
                onClick={() => index === revealedCount && handleCardClick(index)}
                className={`w-24 h-36 cursor-pointer relative perspective-1000 ${
                  index === revealedCount && mode === 'eoa' ? 'animate-bounce' : ''
                }`}
              >
                <motion.div
                  initial={{ rotateY: 0 }}
